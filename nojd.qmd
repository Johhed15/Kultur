...
Denna rapport undersöker kulturen i Uppsala län utifrån invånarnas perspektiv. Syftet är att ge en övergripande bild av hur länets invånare tar del av och upplever kulturlivet, samt i vilken utsträckning kommunerna erbjuder kulturverksamhet och infrastruktur för sina invånare.
...
# Upplevelse av kultur- och nöjesliv

Detta avsnitt redovisar andelen invånare som har svarat ”Mycket bra” eller ”Ganska bra” på frågan ”Vad tycker du om det lokala kultur- och nöjeslivet i kommunen (till exempel konstutställningar, teater, konserter etc.)?” i SCB:s medborgarundersökning. Svarsalternativen ”Vet inte” och ”Ingen åsikt” har exkluderats från beräkningarna.
...
for(r in c("Uppsala län",sort(kommuner))){
...
Detta avsnitt undersöker det faktiska åskådar- och besöksdeltagandet i olika kulturaktiviteter, det vill säga att som publik ta del av kultur, inte att själv utöva eller delta aktivt. Deltagande i kultur redovisas som ett sammanvägt mått baserat på genomsnittligt deltagande i följande aktiviteter:

-   Konstutställning och/eller museum

-   Teater eller bio

-   Sportevenemang

För varje aktivitet avser uppgifterna den genomsnittliga andelen av befolkningen i åldern 16–84 år som deltagit under perioden år $T$ (undersökt år), $T-1$(ett år innan sett från undersökt år) och $T-2$(två år innan). Det innebär att varje värde representerar ett glidande medelvärde över tre år, vilket bidrar till att minska slumpmässiga variationer mellan enskilda år.
...
De flesta län uppvisar liknande deltagandenivåer, vilket tyder på en relativt jämn spridning av kulturdeltagande över landet. De två länen som sticker ut mest är Stockholm och Uppsala, som har en något högre genomsnittlig deltagarandel än övriga regioner.
...
for(r in sort(kommuner)){
  ...
  "Figuren visar utvecklingen av totalt antal lån av fysiska medium över tid för",
  r,
  " per år uppdelat på bibliotekstyp."
...
for(r in cols){
  ...
  cat('<img src="', filename_svg, '" alt= "',paste("Figuren visar aningen antal beviljade fritidskort, antal använda fritidskort eller kvoten mellan antal använda dividerat på antal använda fritidskort för länets kommuner.9"), '" style="width:100%; max-width:800px; height:100%;">\n', sep = "")
...
@fig-fritidsforening baseras på data från @regeringfritid och visar det genomsnittliga antalet nedladdade fritidskort per ansluten förening i Uppsala län. För att sätta siffran i ett nationellt sammanhang jämförs länet mot övriga Sveriges län, där mitten av fördelningen samt de 25 procent lägsta och högsta länen visas.
...
Under år `r df$år` laddades totalt `r df$antal_nedladdade_fritidskort` fritidskort ned i Uppsala län, fördelat på `r df$antal_foreningar` föreningar.

Det innebär i genomsnitt cirka 80 nedladdningar per förening, vilket är påtagligt högre än i de flesta andra län. I hälften av landets län låg motsvarande genomsnitt på mellan 40 och 65 nedladdningar per förening, och Uppsala placerar sig bland de 25 procent av länen med högst antal nedladdningar per förening.
...
for(r in text){
  ...
  cat('<img src="', filename_svg, '" alt= "',paste0("Figuren visar utvecklingen av andelen elever i musik- och kulturskola för de kommunerna med data för åldrarna",r), '" style="width:100%; max-width:800px; height:100%;">\n', sep = "")
...
Att Östhammar har en andel på 0 år 2023 för åldersgruppen 6-19 är troligtvis på grund av att data saknas och har blivit registrerad som 0 av kolada. Knivsta och Älvkarleby saknar data för åren som presenteras ovan och har därav exkluderas från figurerna.
...
```{r}
#| tbl-cap: "Andelen flickor i kulturskolan (6-19 år), anges i procent(%)."
...
Flickor utgör en tydlig majoritet av eleverna i kulturskolan. Endast vid ett tillfälle under hela perioden var pojkarna fler - i Håbo år 2017. I övrigt ligger andelen flickor över 50 procent i samtliga kommuner och år med tillgängliga data. För de kommuner som har aktuella data (de senaste två åren) ligger andelen flickor generellt högt. Uppsala kommun är den enda kommunen under 60 procent med 59,72 procent flickor senaste året.
...
Högst avgift per elev hittas i Håbo, Knivsta och Uppsala kommun. Håbo och Uppsala kommun har en elevavgift på över 1500 kr/elev för en stor del utav åren i perioden. Knivsta har endast data för ett utav åren och redovisar då en elevavgift på över 2500 kr/elev.
