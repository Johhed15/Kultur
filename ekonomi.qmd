...
top <- df[which.max(df$value),]

kom <- top$municipality
```
...
Den kommunen som har högst kostnad per invånare på kulturverksamhet år `r ar` är `r kom`. Det är också den enda kommunen i länet som har en högre kostnad per invånare än rikssnittet.
